...
var deco_user = new XMLHttpRequest();
...
var get_rss = new XMLHttpRequest();
var isSetFav = false;
...
verif_user.onreadystatechange = function()
{
	...
	{
		if(verif_user.responseText == 'no' || verif_user.responseText == '')
		{
...
document.getElementById('gm_recent').addEventListener('click',function(){
	show_listes('rv');
},false);
document.getElementById('dico_addword').addEventListener('click',function(){
	changePage('addword');
	document.getElementById('addtrad_personal').innerHTML = '<div><input type="text" value="" class="glo_in" placeholder="Entrez une traduction..."/></div>';
},false);
...
document.getElementById('changeword_addtrad').addEventListener('click',function(){
	if(document.getElementById('changeword_trad').getElementsByTagName('input')[document.getElementById('changeword_trad').getElementsByTagName('input').length-1].value != '')
	{
		...
		document.getElementById('changeword_trad').innerHTML += '<input type="text" data-id="" value="" class="glo_in" placeholder="Traduction..."/>';
...
document.getElementById('new_pl').addEventListener('click', function(){
	document.getElementById('pl_content').innerHTML = "<h2>Ajout playlist</h2><input id='newpl_name' class='glo_in' type='text' name='name' placeholder='Nom playlist'><div id='submit_addpl' class='btn_valid' onclick='ajoutPl()'>Ajouter</div>";
});
...
document.getElementById('decon').addEventListener('click',function(){
	deco_user.open('POST',url)
	deco_user.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	deco_user.send('id='+userid+'&req=deco');
	deco_user.onreadystatechange = function()
	{
		if(deco_user.readyState == 4)
		{
			window.location = './';
		}
	}
}, false);
document.getElementById('gm_rssfeed').addEventListener('click',function(){
	changePage('rssfeed');
	if(document.getElementById('rss_content').innerHTML == "")
	{
		getRss('http://www.independent.co.uk/news/uk/rss',function(){
			getRss('https://observer.com/feed/',function(){
				getRss('https://rss.nytimes.com/services/xml/rss/nyt/Europe.xml',function(){
					getRss('https://www.theguardian.com/world/rss');
				});
			});
		});
		/*getRss('./rss.txt',function(){
			getRss('./rss.txt');
		});*/
	}
}, false);
function getRss(rssurl,feedback)
{
	rss_content = document.getElementById('rss_content');
	get_rss.open('GET',"./php/getrss.php?url="+encodeURI(rssurl));
	get_rss.send();
	get_rss.onreadystatechange = function()
	{
		if(get_rss.readyState == 4)
		{
			feed = JSON.parse(get_rss.responseText);
			str = '<article class="rss_feed"><h3>'+feed.title+'</h3>';
			var i = 0;
			for(i = 0;i < feed.news.length;i++)
			{
				str += '<a class="rss_item" target="_blank" href="'+feed.news[i].link+'"><h4>'+feed.news[i].title+'</h4><span class="rss_date">'+feed.news[i].date+'</span></a>';
			}
			str += '</article>';
			rss_content.innerHTML += str;
			if(feedback)
			{
				feedback();
			}
...
function getWord(container,displayFav,dispayLet){
	if(allMyWord != '')
	{
		...
		document.getElementById(container).innerHTML = '<div class="dico_allfav mots"></div>';
		document.getElementById(container).innerHTML += "<div class='non_fav mots'></div>";
		var i = 0;
		var fav = '';
		if(allMyWord.length > 0 && allMyWord[0]['favorite'] == 1 && displayFav)
		{
			document.getElementById(container).getElementsByClassName('dico_allfav')[0].innerHTML += '<div class="titre_mots">Favoris</div>';
			document.getElementById(container).getElementsByClassName('non_fav')[0].innerHTML +='<div class="titre_mots">All</div>';
		}
		for(i = 0;i < allMyWord.length;i++)
		{
			...
			{
				document.getElementById(container).getElementsByClassName('non_fav')[0].innerHTML += '<div class="dico_oneletter">'+allMyWord[i]['content'][0].toUpperCase()+'</div>';
				lastLetter = allMyWord[i]['content'][0];
			}
			if(allMyWord[i]['content'][0] != lastLetter && allMyWord[i]['favorite'] == 1 && dispayLet)
			{
				document.getElementById(container).getElementsByClassName('dico_allfav')[0].innerHTML += '<div class="dico_oneletter">'+allMyWord[i]['content'][0].toUpperCase()+'</div>';
				lastLetter = allMyWord[i]['content'][0];
			}
			fav = '';
			...
			{
				fav = 'fav-ok';
				document.getElementById(container).getElementsByClassName('dico_allfav')[0].innerHTML += '<div class="dico_oneword its_fav" id="wordnb'+i+'" onclick="changeWord(this);" data-comm="'+allMyWord[i]['commentaire']+'"><p class="dico_ow_main">'+allMyWord[i]['content']+'</p> <p class="dico_ow_trad">'+allMyWord[i]['trad']+'</p><span onclick="setWordFav(\'wordnb'+i+'\','+allMyWord[i]['favorite']+')" class="dico_ow_fav '+fav+'"></span></div>';
			}
			else
			{
				document.getElementById(container).getElementsByClassName('non_fav')[0].innerHTML += '<div class="dico_oneword" id="wordnb'+i+'" onclick="changeWord(this);" data-comm="'+allMyWord[i]['commentaire']+'"><p class="dico_ow_main">'+allMyWord[i]['content']+'</p> <p class="dico_ow_trad">'+allMyWord[i]['trad']+'</p><span onclick="setWordFav(\'wordnb'+i+'\','+allMyWord[i]['favorite']+')" class="dico_ow_fav '+fav+'"></span></div>';
...
function changeWord(elem)
{
	...
	else
	{
		...
		{
			document.getElementById('changeword_trad').innerHTML += '<input type="text" class="glo_in" data-id="'+allTrad[i]+'" value="'+allTrad[i]+'" placeholder="Traduction..."/>';
		}
	}
}
...
{
	document.getElementsByClassName('pact')[0].setAttribute('class','page');
	document.getElementById('p_'+pageName).setAttribute('class','page pact');
...
function newQuizzSlide()
{
	...
	if(randint(0,4) == 0)
	{
		document.getElementById('quizz_content').innerHTML = str+'<input class="quizz_text_rep glo_in" placeholder="Traduction..."∕><div class="quizz_onerep">Ok</div></div>';
