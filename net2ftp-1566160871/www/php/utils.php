<?php
	...
	if(isset($_POST) && isset($_POST['req']))
	{
		try
		{
			$bdd = new PDO('mysql:host=je47502-001.privatesql:35531;dbname=dicoti;charset=utf8','admin','Aikene1097');
		}
		catch(Exeption $e)
		{
			die($e->getMessage());
			...
			switch($_POST['req'])
			{
				case 'getmyword':
					...
					else
					{
						...
						$dico = $dico->fetch();
						$myword = $bdd->query('SELECT content, trad, favorite, commentaire FROM words WHERE dico_id = '.$dico['id'].' and content <> "" and trad <> "" ORDER BY favorite DESC, content LIMIT 100');
					}
				...
				case 'getstat':
					...
					$t = $t->fetch();
					$graph = $bdd->query('SELECT time as t, nbmotS as nm, nbtrue as nt, DATE_FORMAT(datet,"%d/%m") as d FROM quizz WHERE user_id = 1 ORDER BY datet ASC LIMIT 20');				
					...
					{
						echo 'Nous n\'avons pas encore assez de données pour afficher vos statistiques<div class="hidden" id="stat_graph_data"></div>';
					}
					break;
				case 'savequizz':
